...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow
    {
                ...
                foreach (XNode.NodePort output in node.Outputs)
                {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++)
                    {
                        ...
                        if (input.node == output.node)
                            gridPoints.AddRange(AddAroundPath(fromRect, node, 0, toRect.center.y));
                        if (input.node == output.node)
                            gridPoints.AddRange(AddAroundPath(toRect, node, 1, toRect.center.y));
        ...
        private IEnumerable<Vector2> AddAroundPath(Rect rect, Node node, int num, float y)
        {
            Vector2 pos = node.position;
            var size = node.size; 
            {
                var p = new Vector2(rect.center.x, rect.center.y + 20 * (num == 0 ? 1 : -1));
                var ddy = (y - node.position.y) / 6;
                if (num == 1)
                {
                    yield return new Vector2(node.position.x - 7, node.position.y + size.y + ddy);
                }
                if (num == 0)
                    yield return new Vector2(node.position.x + size.x + 0, node.position.y + size.y + ddy);
            }
        }
